                        ...
                        <form action="{{ route('admin.banner.update',$data->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="modal-body pd-20 row">
                                <div class="form-group col-lg-12">
                                    ...
                                    <input type="text" class="form-control @error('title') is-invalid @enderror" name="title" id="title" value="{{ $data->title }}" aria-describedby="emailHelp" placeholder="Banner Title">
                                    <!--- Error Message -->
                                    @error('title')
                                ...
                                <div class="form-group col-lg-6">
                                    <img src=" @if(isset($data->image)){{ asset('images/'.$data->image) }} @else {{ asset('backend/img/camera.png') }} @endif"
                                        alt="banner_image" id="profile_photo" style="height: 100px;width: 200px;">
                                </div>

                                <div class="form-group col-lg-12">
                                    <label for="description">Banner Description :</label>
                                    <textarea type="text" class="form-control @error('description') is-invalid @enderror" name="description" id="summernote" aria-describedby="emailHelp" placeholder="Banner Description">{!! $data->description !!}</textarea>
                                    <!--- Error Message -->
                                    @error('description')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                ...
                                <div class="form-group col-lg-6">
                                    ...
                                    <select name="condition" id="condition" class="form-control @error('description') is-invalid @enderror" aria-label="Default select example">
                                        ...
                                        <option value="promo" {{ $data->condition=='promo' ? 'selected' : '' }}>Promo</option>
                                    </select>
                                </div>
                            ...
                            <div class="modal-footer">
                                <button type="submit" class="btn btn-info btn-block pd-x-20">Update</button>
                            </div>
                        </form>
                      </div>
                </div>
